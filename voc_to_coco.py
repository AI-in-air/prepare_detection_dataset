...
def voc2coco(root_dir, txt_file, save_name=None, save_dir=None, save_txt=False):
    ...
    txt_list = moyan.readTxt2Lines(txt_file)
    parseXmlFiles(xml_dir, txt_list)
    json.dump(coco, open(save_path, 'w'))
    if save_txt:
        moyan.writeLines2Txt(voc_txt_lines, os.path.join(save_dir, sname.strip().replace('.json', '.txt')))



if __name__ == '__main__':

    root_dir = r'D:\Dataset\VOC\VOCdevkit\VOC2007'
    txt_file = r'D:\Dataset\VOC\VOCdevkit\VOC2007\ImageSets\Main\val.txt'
    save_name = 'val'
